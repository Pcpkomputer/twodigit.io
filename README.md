...
## 🎦 OBS Studio Streaming Overlays

Twodigit provides three ultra-lightweight, customizable browser source overlays designed specifically for OBS Studio, Streamlabs, and vMix.

### 1. 🔔 Real-time Donation Alert Overlay (`/overlay`)
Displays live animated donation cards with YouTube Media Share support and native Indonesian female Text-to-Speech (`id-ID-GadisNeural`):
```text
...
### 2. 🎯 Milestone / Donation Goal Overlay (`/overlay/milestone`)
Displays a live animated progress bar tracking your donation targets, with optional start date filtering and real-time Pusher updates.

**OBS Browser Source URL:**
```text
https://your-domain.vercel.app/overlay/milestone?target=5000000&title=Upgrade+PC+Stream&startDate=2026-08-01
```

#### Query Parameters:
| Parameter | Type | Required | Default | Description | Example |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `target` | `number` | Yes | `1000000` | Target milestone amount in IDR | `target=5000000` |
| `title` | `string` | No | `Donation Goal` | Title displayed above the progress bar | `title=Beli+Mic+Baru` |
| `startDate` | `string` | No | *(All time)* | Filter donations from a specific date (`YYYY-MM-DD` or ISO) | `startDate=2026-08-01` |

---

### 3. 🏆 Top Donors Leaderboard Overlay (`/overlay/top-donors`)
Displays a live leaderboard ranking your top supporters by total donation amount, with rank medals (👑 Gold, 🥈 Silver, 🥉 Bronze) and donor count.

**OBS Browser Source URL:**
```text
https://your-domain.vercel.app/overlay/top-donors?limit=5&title=Top+Donatur+Bulan+Ini&startDate=2026-08-01
```

#### Query Parameters:
| Parameter | Type | Required | Default | Description | Example |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `limit` | `number` | No | `5` | Number of top donors to display | `limit=3` or `limit=10` |
| `title` | `string` | No | `Top Donatur` | Header title of the leaderboard card | `title=Top+Sultan+Stream` |
| `startDate` | `string` | No | *(All time)* | Filter donations starting from this date (`YYYY-MM-DD` or ISO) | `startDate=2026-08-01` |

---

## 🗄️ Database Setup (Supabase)

To enable milestone goals and historical top donor tracking, execute this SQL script in your **Supabase Dashboard ➡️ SQL Editor**:
...
-- Indices for rapid date filtering
